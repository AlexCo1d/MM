...
class Former_cls(Blip2Base):
    ...
    def __init__(
        ...
        if dataloader is not None:
            ...
            self.answer_tokens = self.tokenizer(
                answer_list,
                truncation=True,
    ...
    def forward(self, samples, dataloader=None, alpha=None):
        ...
        # sim_q2a = torch.einsum('b q d, a d -> b a q', query_feats, answer_feats).mean()
    ...
    def predict_answers(
        ...
        pred = sim_q2a.argmax(dim=-1)
        answers = [answer_list[p] for p in pred]
        return answers
        #
